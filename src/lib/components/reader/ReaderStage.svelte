<script lang="ts">
    import { AnnotationController } from "$lib/reader/annnotate.svelte";
    ...
    $effect(() => {
        ...
        annotations.attach(reader, id);

        const off = reader.onSection((doc, index) => annotations.observe(doc, index));
        return () => {
            off();
            annotations.detatch();
        };
...
<div class="stage" class:shaded={reader.showStacks}>
    {#if settings.flow === "paginated"}
        <button class="nav left" onclick={reader.prev} aria-label="Previous page">‹</button>
    {/if}
    <div class="book" style="width:{reader.bookWidthRem}rem">
        {#if reader.showStacks}
            <div class="stack left" style="width:{reader.leftStackPx}px" aria-hidden="true"></div>
        {/if}
        ...
        {#if reader.showStacks}
            <div class="stack right" style="width:{reader.rightStackPx}px" aria-hidden="true"></div>
        {/if}
    </div>
    {#if settings.flow === "paginated"}
        <button class="nav right" onclick={reader.next} aria-label="Next page">›</button>
    {/if}
</div>
...
<style>
    ...
    .spine {
        ...
        z-index: 5;
        pointer-events: none;
        background: linear-gradient(to right, rgba(0, 0, 0, 0) 0%, rgba(0, 0, 0, 0.05) 38%, rgba(0, 0, 0, 0.16) 48%, rgba(0, 0, 0, 0.22) 50%, rgba(0, 0, 0, 0.16) 52%, rgba(0, 0, 0, 0.05) 62%, rgba(0, 0, 0, 0) 100%);
    }
    ...
    .stack {
        ...
        background-image: repeating-linear-gradient(to right, color-mix(in srgb, var(--fg) 10%, transparent) 0 1px, transparent 1px 2.5px);
